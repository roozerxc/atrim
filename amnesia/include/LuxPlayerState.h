...
{
public:
    static void SetupHandObject(const tString& asHandObject)
    {
        msHandObject = asHandObject;
    }
    static void SetupInteraction(iPhysicsBody *apBody, const cVector3f &avInteractPos);
    static void SetupUseItem(cLuxInventory_Item *apItem)
    {
        mpInventoryItem = apItem;
    }
    static void SetupLadder(cLuxArea_Ladder *apLadder, const cVector3f &avStartPos)
    {
        mpLadder = apLadder;
        mvLadderStartPos = avStartPos;
    }
...
public:
    ...
    virtual bool OnMove(eCharDir aDir, float afMul)
    {
        return true;
    }

    virtual bool OnAddYaw(float afAmount)
    {
        return true;
    }
    virtual bool OnAddPitch(float afAmount)
    {
        return true;
    }

    virtual bool OnRun(bool abPressed)
    {
        return true;
    }
    virtual bool OnCrouch(bool abPressed)
    {
        return true;
    }
    virtual bool OnJump(bool abPressed)
    {
        return true;
    }
    ...
    virtual bool OnDrawCrossHair(cGuiGfxElement *apGfx, const cVector3f& avPos, const cVector2f &avSize)
    {
        return true;
    }
    ...
    void SetPreviousState(eLuxPlayerState aState)
    {
