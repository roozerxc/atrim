...
public:
    ...
    inline cVector2f GetSizeFloat2D() const
    {
        return cVector2f((float)mvSize.x, (float)mvSize.y);
    }
    inline cVector2l GetSizeInt2D() const
    {
    ...
    bool IsCompressed()
    {
        return mbIsCompressed;
    }

    void SetSizeDownScaleLevel(unsigned int alLevel)
    {
        mlSizeDownScaleLevel = alLevel;
    }
    void SetMinLevelSize(const cVector2l& avSize)
    {
        mvMinDownScaleSize = avSize;
    }

    eFrameBufferAttachment GetFrameBufferAttachmentType()
    {
