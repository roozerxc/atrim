...
{
	//hpl::Log(" OAL: Begin Destroying source %p\n", this);
	...
	//hpl::Log(" OAL: Done Destroying source %p\n", this);
}
...
double cOAL_Source::GetElapsedTime()
{
	...
	//hpl::Log(" OAL: Getting elapsed time from source %p audiodata: %p  playing: %d\n",this,mpAudioData, mbPlaying);
...
int	cOAL_Source::BindData(cOAL_Sample *apSample)
{
	...
	//hpl::Log(" OAL: Adding audiodata %p ('%s') as sample to source %p\n", mpAudioData,hpl::cString::To8Char(mpAudioData->GetFilename()).c_str(), this);
...
int	cOAL_Source::BindData(cOAL_Stream *apStream)
{
	...
	//hpl::Log(" OAL: Adding audiodata %p ('%s') as stream to source %p\n", mpAudioData,hpl::cString::To8Char(mpAudioData->GetFilename()).c_str(), this);
...
void cOAL_Source::Stop ( bool abRemove )
{
	...
	if(mpAudioData)
	{
		//hpl::Log(" OAL: Removing audiodata %p ('%s') from source %p remove: %d\n", mpAudioData,hpl::cString::To8Char(mpAudioData->GetFilename()).c_str(), this, abRemove);
...
void cOAL_Source::Update()
{
	...
	{
		//hpl::Log("Source %d playing %ls is having a buffer underrun at %f/%f\n", mlId, mpAudioData->GetFilename().c_str(), GetElapsedTime(), GetTotalTime());
