...
static cVector3f GetAxisVector(int alAxis, float afValue)
{
    ...
    {
    case 0:
        return cVector3f(afValue,0,0);
    case 1:
        return cVector3f(0,afValue,0);
    case 2:
        return cVector3f(0,0,afValue);
    }
    ...
    for(int axis=0; axis<3; axis++)
    {
        for(int index = 0; index < (int)apSrcVec[axis].size(); index++)
        {
            ...
            if(lPos>=0)
            {
                ...
                {
                case eAnimTransformFlag_Translate:
                    (*apDestVec)[lPos].vTrans += vVec;
                    break;
                case eAnimTransformFlag_Scale:
                    (*apDestVec)[lPos].vScale += vVec;
                    break;
                case eAnimTransformFlag_Rotate:
                    (*apDestVec)[lPos].vRot += vVec;
                    break;
                }
            }
            else
...
const char*  cMeshLoaderFBX::GetSkelTypeName(KFbxSkeleton::ESkeletonType alNum)
{
    switch(alNum)
    {
    case KFbxSkeleton::eROOT:
        return "Root";
    case KFbxSkeleton::eLIMB:
        return "Limb";
    case KFbxSkeleton::eLIMB_NODE:
        return "Limb Node";
    case KFbxSkeleton::eEFFECTOR:
        return "Effector (root)";
    }
